...
c***first executable statement  dgeco
      anorm = 0.0d0
      do 10 j = 1, n
         anorm = dmax1(anorm,dasum(n,a(1,j),ione))
   10 continue
c
c     factor
c
...
c
      do 120 kb = 1, n
         k = n + 1 - kb
         if (k .lt. n) z(k) = z(k) + ddot(n-k,a(k+1,k),ione,z(k+1),ione)
...
c
      do 140 k = 1, n
         ...
         z(l) = z(k)
         z(k) = t
         if (k .lt. n) call daxpy(n-k,t,a(k+1,k),ione,z(k+1),ione)
...
c
  ...
  150    continue
         ...
         t = -z(k)
         call daxpy(k-1,t,a(1,k),ione,z(1),ione)
  160 continue
c     make znorm = 1.0
      s = 1.0d0/dasum(n,z,ione)
      call dscal(n,s,z,ione)
